import { listAddressedPlaces } from "@/lib/locations";
...
export default async function AddressesPage() {
  const shelves = await listAddressedPlaces();
    ...
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col gap-5 px-4 pt-6 pb-6">
      ...
      {shelves.length > 0 ? (
        ...
        <p className="rounded-xl border border-dashed border-line p-6 text-center text-sm text-muted">
          No addresses yet. Add a shelf, or give a section an address, from the
          Tree view and it will appear here.
        </p>
      )}
    </main>
  );
